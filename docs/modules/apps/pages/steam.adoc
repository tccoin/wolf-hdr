...
Steam's own library, Store and Big Picture UI are SDR surfaces.  For an HDR client, the correct
arrangement is to run Steam inside a Gamescope Steam session (`gamescope -e --hdr-enabled`):
Gamescope exposes its HDR capability to Steam and maps those SDR UI surfaces to the configured
SDR reference white, while HDR-capable Proton games submit their own PQ surface.  Do not enable
Gamescope HDR inverse tone mapping for this path: inverse tone mapping is only for SDR input and
changes the appearance of native HDR games.

The Wolf `Steam HDR` launcher is transport-aware.  It starts Gamescope HDR only when the
connected Moonlight client negotiated HDR; otherwise it uses Wolf's native SDR output path.
It intentionally does not set `PROTON_ENABLE_WAYLAND` globally because that experimental Wine
driver can break Steam Input.  The normal Gamescope/XWayland route remains the default.
It also uses the same `STEAM_GAMESCOPE_COLOR_MANAGED` and
`STEAM_GAMESCOPE_VIRTUAL_WHITE` declarations as a SteamOS Gamescope session, with SDR gamut
widening disabled.  This keeps Steam UI and SDR-only games in sRGB instead of treating them as
wide-gamut HDR content.

=== Games that do not expose an HDR option

First use a current Proton version and launch the game from the HDR Steam session.  For a title
that still does not expose HDR, add this to *that game's* Steam launch options:
...
The wrapper enables `DXVK_HDR`, `ENABLE_GAMESCOPE_WSI`, `ENABLE_HDR_WSI` and
`PROTON_ENABLE_HDR` only for an HDR Moonlight connection.  It is a no-op HDR-wise for SDR
connections, so a per-game setting cannot accidentally send PQ into Wolf's SDR encoder.

[NOTE]
====
Only add `PROTON_ENABLE_WAYLAND=1` to a single game's launch options if that title needs the
experimental Wine Wayland driver and controller input has been tested.  It is not a general HDR
switch.
====
...
From where your `/etc/wolf` is mounted in your host, add the desired Proton files to `/etc/wolf/profile_data/${profile_id}/WolfSteam/.steam/compatibilitytools.d/`. This is the compatibility-tool directory scanned by Steam in the Wolf profile; placing tools under `.steam/steam/compatibilitytools.d/` will not register them.
...
* Extract the release tarball into `/etc/wolf/profile_data/${profile_id}/WolfSteam/.steam/compatibilitytools.d/`
* Restart Steam
